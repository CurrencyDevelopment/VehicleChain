...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #MVDSCoinTEST2\r");
            Send(hSocket, "WHO #MVDSCoinTEST2\r");
        } else {
            // randomly join #MVDSCoin00-#MVDSCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #MVDSCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #MVDSCoin%02d\r", channel_number).c_str());
        }
