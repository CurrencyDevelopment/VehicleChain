MVDSCoin-qt: Qt4 GUI for MVDSCoin
===============================
...
Alternatively, install Qt Creator and open the `MVDSCoin-qt.pro` file.

An executable named `MVDSCoin-qt` will be built.
...
A warning for people using the *static binary* version of MVDSCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of MVDSCoin is linked against libdb5.3.
...
installed causes MVDSCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
